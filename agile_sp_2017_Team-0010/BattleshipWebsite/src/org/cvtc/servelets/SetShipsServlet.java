...
public class SetShipsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		CarrierShip playerCarrierShip = new CarrierShip(carrierShipStart[0], Integer.parseInt(carrierShipStart[1]), Integer.parseInt(carrierShipStart[2]));
		
		BattleShip playerBattleShip = new BattleShip(battleShipStart[0], Integer.parseInt(battleShipStart[1]), Integer.parseInt(battleShipStart[2]));
	
		SubShip playerSubShip = new SubShip(subShipStart[0], Integer.parseInt(subShipStart[1]), Integer.parseInt(subShipStart[2]));
	
		DestroyerShip playerDestroyerShip = new DestroyerShip(destroyerShipStart[0], Integer.parseInt(destroyerShipStart[1]), Integer.parseInt(destroyerShipStart[2]));
	
		PBShip playerPbShip = new PBShip(pbShipStart[0], Integer.parseInt(pbShipStart[1]), Integer.parseInt(pbShipStart[2]));
		...
		try {
			...
			response.getWriter().append("Ships Set");
			
		} catch (Exception e) {
			
			response.getWriter().append("Ships Set Error" + e.getMessage());
